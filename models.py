...
class voxels_to_z(nn.Module):
    def __init__(self, num_voxels, z_dim):
        ...
        self.layers = nn.Sequential(nn.Linear(num_voxels, 256),
                                   nn.LeakyReLU(0.1),
                                   nn.Linear(256, z_dim),
                                   nn.LeakyReLU(0.1),
                                   FC_ResNet(z_dim),
...
class fMRI_to_BigGAN(nn.Module):
    def __init__(self, num_voxels, z_dim):
        super(fMRI_to_BigGAN, self).__init__()
        self.z_layer = voxels_to_z(num_voxels, z_dim)
        self.BigGAN = Generator(96, dim_z=z_dim, shared_dim=128, hier=True)
        
    def forward(self, z, y):
        z = self.z_layer(z)
        return self.BigGAN(z, self.BigGAN.shared(y))
